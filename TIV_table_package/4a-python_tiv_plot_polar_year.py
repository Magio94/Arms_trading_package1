...
top_imp_values = imp_tiv.nlargest(n_country, str(year_input))
top_exp_values = exp_tiv.nlargest(n_country, str(year_input))
...
fig = px.bar_polar(selected_data, theta="Country", r=str(year_input),template = "plotly_dark", color=selected_data[str(year_input)])

fig.update_layout(height=600, showlegend=True)
fig.update_layout(showlegend=False,
                  title_text="STOCKHOLM INTERNATIONAL PEACE RESEARCH INSTITUTE<br>" +
                      "<i>Arms " + str(type_input) + " : " + str(year_input) + " Trend Indicator Values<i><br><br>")
